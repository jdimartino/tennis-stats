...
let leagueId = urlParams.get('equipo') || localStorage.getItem('adminLeagueId') || '';

// Save to localStorage if we got it from URL
if (urlParams.get('equipo')) {
...
async function inicializarAdmin() {
    try {
        ...
        // Cargar Equipos primero (para el contexto)
        await cargarEquiposAdmin();
        configurarSelectorContexto();

        // Si no hay equipo seleccionado, forzar selección o tomar el primero
...
function mostrarSeccion(nombreSeccion) {
    ...
    } else if (nombreSeccion === 'equipos') {
        mostrarEquiposGestion();
    }
}

/**
 * Configura el selector de contexto de equipo
 */
function configurarSelectorContexto() {
    ...
    selector.addEventListener('change', async (e) => {
        ...
        // Update UI title with new team name
        await actualizarTituloEquipo();
    });
}
...
function mostrarClubesGestion() {
    ...
    const clubesArray = Object.values(clubesAdmin).sort((a, b) => {
        // "Club Táchira" siempre primero
        if (a.nombre === "Club Táchira") return -1;
        if (b.nombre === "Club Táchira") return 1;
...
async function cargarHistorialGestion() {
    ...
    try {
        ...
        container.innerHTML = partidos.map(p => {
            ...
            // Tipo de partido con etiqueta visual
            const tipoPartido = p.tipoPartido || 'amistoso';
            const tipoLabel = tipoPartido === 'cta' ? '🏆 CTA' : '🤝 Amistoso';
            const tipoColor = tipoPartido === 'cta' ? '#F59E0B' : '#3B82F6';

            return `
                <div class="match-card" style="margin-bottom: 1rem; border: 1px solid var(--color-border);">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.75rem;">
                        <div style="display: flex; gap: 0.75rem; align-items: center;">
                            <span style="font-size: 0.75rem; font-weight: 700; color: ${tipoColor}; background: ${tipoColor}20; padding: 0.25rem 0.5rem; border-radius: 4px;">${tipoLabel}</span>
                            <span style="font-size: 0.8rem; font-weight: 700; color: var(--color-text-muted);">📅 ${fecha}</span>
                        </div>
...
async function prepararEdicionMatch(id) {
    try {
        ...
        // Tipo de partido (con valor por defecto para partidos antiguos)
        const tipoPartido = data.tipoPartido || 'amistoso';
        document.getElementById('tipo-amistoso').checked = (tipoPartido === 'amistoso');
        document.getElementById('tipo-cta').checked = (tipoPartido === 'cta');
...
function configurarFormularios() {
    ...
    document.getElementById('form-partido')?.addEventListener('submit', async (e) => {
        ...
        const tipoPartido = document.querySelector('input[name="tipo-partido"]:checked').value;
...
window.cargarClubesPredefinidos = cargarClubesPredefinidos;
...
/**
 * Carga inicial de clubes predefinidos
 */
async function cargarClubesPredefinidos() {
    const clubes = [
        "Club Táchira",
        "Altamira Tennis Club",
        "Caracas Country Club",
        "Caracas Racquet Club",
        "Caracas Sports Club",
        "Caracas Theater Club",
        "Centre Català",
        "Centro Asturiano De Caracas",
        "Centro Deportivo Hebraica",
        "Centro Portugués",
        ...
        "Monte Claro Country Club",
        "Valle Arriba Athletic Club",
        "Valle Arriba Golf Club"
    ];

    if (!confirm(`¿Estás seguro de que quieres agregar ${clubes.length} clubes predefinidos? Esto podría duplicar clubes si ya existen.`)) {
        return;
    }

    let agregados = 0;
    const existingClubNames = Object.values(clubesAdmin).map(c => c.nombre.toLowerCase());

    for (const nombre of clubes) {
        if (existingClubNames.includes(nombre.toLowerCase())) {
            console.log(`Club ya existe: ${nombre}`);
            continue;
        }
...
/**
// ============================================
// GESTIÓN DE EQUIPOS (Antes Ligas)
// ============================================

/**
 * Carga todos los equipos (ligas) desde Firestore
 */
async function cargarEquiposAdmin() {
    try {
        ...
        // Actualizar selector de contexto si existe
        // se hace en configurarSelectorContexto
    } catch (error) {
...
/**
 * Muestra la lista de equipos en la sección de gestión
 */
function mostrarEquiposGestion() {
    const container = document.getElementById('equipos-lista');
    if (!container) return;

    const items = Object.values(ligasAdmin).sort((a, b) => a.nombre.localeCompare(b.nombre));

    if (items.length === 0) {
        container.innerHTML = `<div class="p-4 text-center">No hay equipos registrados.</div>`;
        return;
    }
    ...
    <div class="club-list">
      ${items.map(item => `
        <div class="club-item">
          <div class="player-info">
             <div>${item.nombre}</div>
             <small style="color: var(--color-text-muted);">${item.active ? '🟢 Activo' : '🔴 Inactivo'}</small>
          </div>
          <div style="display: flex; gap: 0.5rem;">
            <button class="btn btn-action btn-action-edit" onclick="alternarEstadoEquipo('${item.id}', ${!item.active})" title="${item.active ? 'Desactivar' : 'Activar'}">
                ${item.active ? '🚫' : '✅'}
            </button>
            <button class="btn btn-action btn-action-danger" onclick="eliminarEquipo('${item.id}')" title="Borrar">
                🗑️
            </button>
          </div>
        </div>
...
async function agregarEquipo(nombre) {
    try {
        ...
        // Actualizar selector de contexto global
        configurarSelectorContexto();
...
// Event Listeners para formularios de Equipos
document.addEventListener('DOMContentLoaded', () => {
    // ... otros listeners ...
    ...
    if (formEquipo) {
        formEquipo.addEventListener('submit', (e) => {
            ...
            if (nombre) agregarEquipo(nombre);
        });
    }
});
